...
def get_event_start_dt(event):
    ...
    If the event's start object includes a timeZone that is 'Pacific Standard Time'
    or 'Pacific Daylight Time', assume the provided dateTime is already in Pacific.
    """
...
def should_ignore_event(event):
    ...
    if organizer is sjb@silvix.org and the subject contains 'reservation confirmed'.
    """
    if not event:
        return True
